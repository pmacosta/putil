...
def test_add_exception_works():
    ...
    for full_cname, exclude in combinations:
        ...
        for exname in cdb:
            ...
            if full_cname and exclude:
                ...
                if re.compile(r'\d+/first_exception').match(exname):
                    ttuple = (
                        template.format('test_add_exception_works.func1'),
                        TypeError,
                        'This is the first exception'
                    )
                elif re.compile(r'\d+/second_exception').match(exname):
                    ttuple = (
                        template.format('test_add_exception_works.func2'),
                        ValueError,
                        'This is the second exception'
                    )
                    ...
                    ttuple = (
                        template.format('test_add_exception_works.func7'),
                        TypeError,
                        'Total exception #7'
                    )
                elif re.compile(r'\d+/total_exception_8').match(exname):
                    ttuple = (
                        template.format('test_add_exception_works.func8'),
                        TypeError,
                        'Total exception #8'
                    )
                elif re.compile(r'\d+/total_exception_9').match(exname):
                    ttuple = (
                        template.format('test_add_exception_works.func9'),
                        TypeError,
                        'Total exception #9'
                    )
                elif re.compile(r'\d+/total_exception_10').match(exname):
                    ttuple = (
                        template.format('test_add_exception_works.func10'),
                        TypeError,
                        'Total exception #10'
                    )
                elif re.compile(r'\d+/total_exception_11').match(exname):
                    ttuple = (
                        template.format('test_add_exception_works.func11'),
                        TypeError,
                        'Total exception #11'
                    )
                elif re.compile(r'\d+/total_exception_12').match(exname):
                    ttuple = (
                        template.format('test_add_exception_works.func12'),
                        TypeError,
                        'Total exception #12'
                    )
                elif re.compile(r'\d+/total_exception_13').match(exname):
                    ttuple = (
                        template.format('test_add_exception_works.func13'),
                        TypeError,
                        'Total exception #13'
                    )
                elif re.compile(r'\d+/total_exception_14').match(exname):
                    ttuple = (
                        template.format('test_add_exception_works.func14'),
                        TypeError,
                        'Total exception #14'
                    )
        ...
        assert (
            exobj._get_callable_full_name(frobj, '<module>', None) == 'dynamic'
        )
        ...
        exname = list(ecb.keys())[0]
        erec = ecb[exname]
        mname = 'tests.test_exh.test_add_exception_works'
        ref = sorted(
                {
                    'function':[
                        '{mname}/{mname}.MyClass.__init__'.format(mname=mname)
                    ],
                    'type':OSError,
                    'msg':'Init exception'
                }.items()
    ...
    with mock.patch(patch_name, side_effect=patch_obj):
        frobj = sys._getframe(0)
        assert (
            obj._get_callable_full_name(frobj, '<module>', fobj) == 'dynamic'
        )
    ###
...
def test_str():
    ...
    for full_cname in [True, False]:
        ...
        for str_element in str_in:
            ...
            if str_list[3].endswith('test_exh.test_str.func7'):
                str_list[3] = 'Function: {0}'.format(
                    'test_exh.test_str.func7' if full_cname else 'None'
                )
            elif str_list[3].endswith('test_exh.test_str.func8'):
                str_list[3] = 'Function: {0}'.format(
        ...
        str_check.append(
            ...
            'Function: {name}'.format(
                name='test_exh.test_str.func7' if full_cname else 'None'
            )
        )
        str_check.append(
            ...
            'Function: {name}'.format(
                name='test_exh.test_str.func8' if full_cname else 'None'
            )
        )
        str_check.append(
