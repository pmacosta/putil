...
def test_exdoc_errors():
	...
	putil.test.assert_exception(obj, {'exh_obj':exobj, 'depth':'hello'}, TypeError, 'Argument `depth` is not valid')
	putil.test.assert_exception(obj, {'exh_obj':exobj, 'depth':-1}, TypeError, 'Argument `depth` is not valid')
	putil.test.assert_exception(obj, {'exh_obj':exobj, 'exclude':-1}, TypeError, 'Argument `exclude` is not valid')
	putil.test.assert_exception(obj, {'exh_obj':exobj, 'exclude':['hello', 3]}, TypeError, 'Argument `exclude` is not valid')
	...
	putil.test.assert_exception(obj, {'exh_obj':exobj, '_no_print':5}, TypeError, 'Argument `_no_print` is not valid')
	putil.exdoc.ExDoc(exobj, depth=1, exclude=[], _step=-1)
...
def test_depth_property():
	...
	def func1():	#pylint: disable=C0111,W0612
		exobj.add_exception('first_exception', TypeError, 'This is the first exception')
	func1()
	obj = putil.exdoc.ExDoc(exobj, _no_print=True, _step=1)
...
def test_build_ex_tree():
	...
	trace_module()
	exdocobj = putil.exdoc.ExDoc(exh_obj=putil.exh.get_exh_obj(), _no_print=True, _step=0)
	putil.exh.del_exh_obj()
	assert str(exdocobj._tobj) == \
		u'test_exdoc.trace_module\n'.encode('utf-8') + \
	...
	putil.test.assert_exception(putil.exdoc.ExDoc, {'exh_obj':exobj, '_no_print':True, '_step':0}, RuntimeError, 'Functions performing actions for multiple properties not supported')
