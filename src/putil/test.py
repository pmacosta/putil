...
def evaluate_value_series(cmd, pairs):
	...
	for num, (value, expected_result) in enumerate(pairs):
		...
		expected_list.append(comp_text.format(num, cmd_name, value, expected_result))
		actual_list.append(comp_text.format(num, cmd_name, value, actual_result))
...
def evaluate_exception_series(cmd, exdesc, offset=0):	#pylint: disable=R0914
	...
	exdesc = exdesc if isinstance(exdesc, list) else list(exdesc)
	cmd_name = full_command_name(cmd)
	# Evaluate pairs and produce readable test list
	expected_list = list()
	actual_list = list()
	...
	for num, (args, extype, exmsg) in enumerate(exdesc):
		...
		expected_list.append(comp_text.format(num+offset, cmd_name, arg_text, ex_text.format(exception_type_str(extype), exmsg)))
