...
class ExHandle(object):
	...
	def ex_add(self, name, parent, funcname, extype, exmsg):	#pylint: disable=R0913
		""" Add exception to database """
		self._ex_list.append({'name':name, 'parent':parent, 'function':funcname, 'type':extype, 'msg':exmsg, 'checked':False})
	...
	def start_trace(self, func_name):
		""" Starts tracing which exceptions are being checked and in which routine """
		if self._trace_on:
			raise RuntimeError('Exception tracing already on')
		self._trace_on = True

	def stop_trace(self):
		""" Stops exception tracing """
		if not self._trace_on:
			raise RuntimeError('Exception tracing not on')
		self._trace_on = False
	...
	def __str__(self):
		ret = ['Name....: {0}\nFunction: {1}\nParent..: {2}\nType....: {3}\nMessage.: {4}\nChecked.: {5}'.format(ex['name'], ex['function'], ex['parent'], self._ex_type_str(ex['type']), ex['msg'], ex['checked']) for ex in self._ex_list]
		return '\n\n'.join(ret)
