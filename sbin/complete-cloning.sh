...
while getopts ":he" opt; do
	case ${opt} in
		...
		e)
			...
			;;
		\?)
			echo "complete_cloning.sh: invalid option" >&2
			print_usage_message
			exit 1
			;;
	esac
...
if [ "${check_email}" == 1 ]; then
	sed -i -r 's/^check_email=[0|1]$/check_email=1/g' ${pkg_dir}/.hooks/pre-commit
else
	sed -i -r 's/^check_email=[0|1]$/check_email=0/g' ${pkg_dir}/.hooks/pre-commit
fi
