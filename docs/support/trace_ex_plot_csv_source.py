...
def trace_module(no_print=True):
	...
	with putil.exdoc.ExDocCxt(exclude=['_pytest', 'execnet']) as exdoc_obj:
		if pytest.main('-x {0}-k TestCsvSource {1}'.format('{0} '.format(noption) if noption else '', os.path.realpath(os.path.join(os.path.dirname(__file__), '..', '..', 'tests', 'test_plot.py')))):
