...
:py:meth:`putil.exh.ExHandle.add_exception` calls are covered (exceptions
generated by contracts defined using the :py:mod:`putil.pcontracts` module are
automatically traced when the contracts are checked). A convenient way of
tracing a module is to simply run its test suite, provided that it covers the
exceptions that need to be documented.
